...
(function() {
    ...
    function x(a, m) {
        ...
        (function() {
            for (
                var e = a.concat(m), l = [], p = {}, d = 0, g = e.length; d < g;
                ++d
            ) {
                var r = e[d],
                    n = r[3];
    ...
    function u(a) {
        ...
        m.push(["pln", /^\s+/, q, " \r\n\t\xa0"]);
        e.push(
            ["lit", /^@[$_a-z][\w$@]*/i, q], ["typ", /^(?:[@_]?[A-Z]+[a-z][\w$@]*|\w+_t\b)/, q], ["pln", /^[$_a-z][\w$@]*/i, q], [
                "lit",
    ...
    function E(a) {
        ...
        try {
            ...
            for (n = r = 0; n < g;) {
                for (
                    var z = d[n], f = d[n + 1], b = n + 2; b + 2 <= g && d[b + 1] === f;
    ...
    window.prettyPrint = function(a) {
        function m() {
            for (
                var e = window.PR_SHOULD_USE_CONTINUATION ? l.now() + 250 : Infinity; p < h.length && l.now() < e; p++
            ) {
                var n = h[p],
                    k = n.className;
